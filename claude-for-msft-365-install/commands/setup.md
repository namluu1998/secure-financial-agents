...
steps. Start a new `## Run — <timestamp>` section and append each sanitized
command and its captured output (IDs, URLs) as you go. Never record secret
argument values in the log: replace `google_client_secret`, `azure_api_key`,
and `gateway_token` values with `<redacted>` before writing a command line.
