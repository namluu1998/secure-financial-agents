...
async function main() {
  ...
  for (const p of pairs) {
    ...
    const displayValue = spec.secret ? "<redacted>" : v;
    if (!spec.pattern.test(v)) console.warn(`warn: ${k}=${displayValue} — expected ${spec.hint}`);
    if (spec.secret) {
      console.warn(
  ...
  console.log(`Wrote ${out} (${host}, configured keys: ${[...params.keys()].join(", ")})`);
}
