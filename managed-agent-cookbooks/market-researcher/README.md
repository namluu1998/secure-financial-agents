...
Your orchestration layer must validate `sector-reader` output against its `output_schema` before downstream use; direct deployment is rejected because the API does not enforce that boundary. `note-writer` produces `./out/primer-<sector>.docx` (and `.pptx` if slides requested).
