...
Your orchestration layer must validate `transcript-reader` output against its `output_schema` before downstream use; direct deployment is rejected because the API does not enforce that boundary. `note-writer` produces `./out/note-<ticker>.docx` and the updated model at `./out/model-<ticker>.xlsx`.
