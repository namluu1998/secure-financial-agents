...
Your orchestration layer must validate `package-reader` output against its `output_schema` before downstream use; direct deployment is rejected because the API does not enforce that boundary. `publisher` produces `./out/lp-pack-<fund>.xlsx`.
