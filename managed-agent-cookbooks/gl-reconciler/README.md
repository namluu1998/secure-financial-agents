...
The `reader` output must be passed through `scripts/validate.py` by your orchestration layer before the orchestrator sees it. Direct deployment is rejected because the API does not enforce this schema boundary. After validation, the `critic` independently re-verifies each break against trusted sources before the orchestrator hands the set to `resolver`. The `resolver` writes the exception report to `./out/`; it never opens an outsider file.
